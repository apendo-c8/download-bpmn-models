...
const getFileIdsByMilestoneTag = async (token) => {
    ...
    try {
        ...
        return response.data.items.map((item) => item.fileId);
    }
    catch (error) {
        (0, core_1.setFailed)(error instanceof Error ? error.message : 'An error occurred');
    }
...
const downloadFiles = async (data, destinationFolderPath, fileName) => {
    try {
        ...
        await promises_1.default.writeFile(destinationFilePath, data);
...
const runWorkflow = async () => {
    try {
        ...
        const fileContent = await getFileContent(token, fileIds);
